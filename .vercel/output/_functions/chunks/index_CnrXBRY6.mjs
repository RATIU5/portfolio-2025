import { C as CredentialsProviderError, F as chain, s as setCredentialFeature, j as getProfileName, G as readFile, H as HttpRequest } from './_.._DIlKm_GA.mjs';
import { createHash, createPrivateKey, createPublicKey, sign } from 'node:crypto';
import { promises } from 'node:fs';
import { homedir } from 'node:os';
import { dirname, join } from 'node:path';
import { p as parseKnownFiles } from './parseKnownFiles_uLvetf4y.mjs';

const resolveCredentialSource = (credentialSource, profileName, logger) => {
    const sourceProvidersMap = {
        EcsContainer: async (options) => {
            const { fromHttp } = await import('./index_sfkRqL-x.mjs');
            const { fromContainerMetadata } = await import('./index_Bgx_WJGi.mjs');
            logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
            return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
        },
        Ec2InstanceMetadata: async (options) => {
            logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
            const { fromInstanceMetadata } = await import('./index_Bgx_WJGi.mjs');
            return async () => fromInstanceMetadata(options)().then(setNamedProvider);
        },
        Environment: async (options) => {
            logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Environment");
            const { fromEnv } = await import('./index_DkPVIiHx.mjs');
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, callerClientConfig, visitedProfiles = {}, resolveProfileData) => {
    ...
    if (!options.roleAssumer) {
        const { getDefaultRoleAssumer } = await import('./index_BxKlUD_3.mjs');
...
class LoginCredentialsFetcher {
    ...
    async refresh(token) {
        const { SigninClient, CreateOAuth2TokenCommand } = await import('./index_B76gGs7k.mjs');
...
const resolveProcessCredentials = async (options, profile) => import('./index_DzVoOXEI.mjs').then(({ fromProcess }) => fromProcess({
    ...options,
    profile,
...
const isWebIdentityProfile = (arg) => Boolean(arg) &&
    ...
    typeof arg.role_arn === "string" &&
    ["undefined", "string"].indexOf(typeof arg.role_session_name) > -1;
const resolveWebIdentityCredentials = async (profile, options, callerClientConfig) => import('./index_CSfdVktV.mjs').then(({ fromTokenFile }) => fromTokenFile({
